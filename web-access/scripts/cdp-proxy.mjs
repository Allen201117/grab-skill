...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /new?url=xxx[&profileDomain=xiaohongshu.com] - 创建新后台 tab
    // profileDomain: 当指定时，在该域名已有 tab 的 profile 里通过 <a> 链接点击打开新 tab（继承登录态）
    else if (pathname === '/new') {
      const targetUrl = q.url || 'about:blank';
      let targetId;

      if (q.profileDomain && targetUrl !== 'about:blank') {
        try {
          const targetsResp = await sendCDP('Target.getTargets');
          const pages = targetsResp.result.targetInfos.filter(t => t.type === 'page');
          const matchTab = pages.find(t => t.url && t.url.includes(q.profileDomain) && t.browserContextId);
          if (matchTab) {
            // 在已有 tab 里用 <a target="_blank"> 点击来继承 profile context
            const sourceId = matchTab.targetId;
            const sid = await ensureSession(sourceId);
            const escaped = JSON.stringify(targetUrl);
            const beforeTargets = new Set((await sendCDP('Target.getTargets')).result.targetInfos.map(t => t.targetId));
            await sendCDP('Runtime.evaluate', {
              expression: `(()=>{const a=document.createElement('a');a.href=${escaped};a.target='_blank';a.rel='opener';document.body.appendChild(a);a.click();document.body.removeChild(a);})()`,
              userGesture: true
            }, sid);
            // 等新 tab 出现
            for (let i = 0; i < 10; i++) {
              await new Promise(r => setTimeout(r, 500));
              const afterTargets = (await sendCDP('Target.getTargets')).result.targetInfos;
              const newTab = afterTargets.find(t => !beforeTargets.has(t.targetId) && t.type === 'page');
              if (newTab) { targetId = newTab.targetId; break; }
            }
            // 新 tab 出现后，明确导航到目标 URL（防止 SPA 拦截 <a> 点击导致路由错误）
            if (targetId) {
              const newSid = await ensureSession(targetId);
              await sendCDP('Page.navigate', { url: targetUrl }, newSid);
              await waitForLoad(newSid);
            }
          }
        } catch (_) { /* 继续走默认流程 */ }
      }
